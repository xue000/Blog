...
class MainTestCase(BaseTestCase):
    ...
    def test_show_post(self):
        response = self.client.get(url_for('main.show_post', post_id=1), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_show_category(self):
        ...
        post = Post(title='test post2', body='Test post2', category=category2)
        response = self.client.get(url_for('main.show_category', category_id=1))
        data = response.get_data(as_text=True)
        self.assertIn('test post', data)
        self.assertNotIn('test post2', data)
